...
{#if open}
  <div class="fixed inset-0 z-40 flex items-start md:items-center justify-center p-2 md:p-6">
    <button type="button" class="absolute inset-0 bg-black/30" aria-label="Close" onclick={() => dispatch('close')}></button>
    <div class="relative z-10 w-full md:w-5/6 lg:w-3/4 xl:w-2/3 max-h-[85vh] overflow-auto bg-white rounded-xl shadow-xl border p-3">
      <div class="flex items-center justify-between border-b pb-2 mb-3">
        ...
        <button type="button" class="px-2 py-1 rounded border text-xs bg-gray-50 hover:bg-gray-100" onclick={() => dispatch('close')}>Close</button>
      </div>
